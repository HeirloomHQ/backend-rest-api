...
def members(memorial_id):
    ...
    return {"members": response}, 200


@memorial_bp.route('/<memorial_id>/members/<role_id>', methods=['PUT'])
@jwt_required()
def put_member(memorial_id, role_id):
    if not request.is_json:
        return {"msg": "Missing JSON in request"}, 400

    user_id = get_jwt_identity()
    response, code = edit.edit_role(user_id, memorial_id, role_id, request.json.get("role", None))
